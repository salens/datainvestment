                ...
                <?php if( $fortun_options['header-top-bar'] == '1' && $fortun_options['header-menu-style'] != 'side-header-menu' ){ ?>
                    <div class="header-top-bar <?php 
                        ...
                        <div class="container<?php if( $header_menu_fullwidth == '1'){ echo '-fluid'; } ?>">
                            <?php if( !empty($fortun_options['header-top-email']) ){ ?><span class="top-bar-email"><i class="fa fa-envelope-o"></i><?php echo esc_html($fortun_options['header-top-email']); ?></span><?php } ?>
                            <?php if( !empty($fortun_options['header-top-number']) ){ ?><span class="top-bar-number"><i class="fa fa-mobile-phone"></i><?php echo esc_attr($fortun_options['header-top-number']); ?></span><?php } ?>
...
function agni_posts( $atts = null, $archive = null, $shortcode = null ){ 
    ...
    $col = $post_additional_class = $i = $delay = '';
...
function fortun_register_required_plugins() {
    ...
    $plugins = array(
        ...
        array(
            ...
            'source'                => AGNI_FRAMEWORK_DIR . '/template/plugins/envato-wordpress-toolkit 1.7.3.zip', 
            'required'              => false,
            'version'               => '1.7.3', 
            'force_activation'      => false,
            'force_deactivation'    => false, 
            'external_url'          => esc_url( 'https://github.com/envato/envato-wordpress-toolkit' ),
        ),
